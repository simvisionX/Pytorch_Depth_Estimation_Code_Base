...
class ToTensor(object):
    ...
    def __call__(self, sample):
        left = np.transpose(sample['left'], (2, 0, 1))
...
class RandomCrop(object):
    ...
    def __call__(self, sample):
        ...
        # TODO：这里只覆盖了两种情况，没有覆盖到crop一大一小的情况，源码同样也没有，后续有需要的话可以改一下
        if self.img_height > ori_height and self.img_width > ori_width:
            ...
            sample['right'] = np.lib.pad(sample['left'],
                                        ((top_pad, 0), (0, right_pad), (0, 0)),
                                        mode='constant',
                                        constant_values=0)
            ...
            if not self.validate:
                self.offset_x = np.random.randint(ori_width - self.img_width)
                self.offset_y = np.random.randint(ori_height - self.img_height)
    ...
    def crop_img(self, img):
        return img[self.offset_y:self.offset_y + self.img_height, self.offset_x: self.offset_x + self.img_width]



class RandomVerticalFlip(object):
    # Randomly vertically flips
